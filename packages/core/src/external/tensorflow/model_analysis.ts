// Taken from (with some modifications):
// https://github.com/tensorflow/tfjs/blob/master/tfjs-converter/src/executor/model_analysis.ts
...
import { Graph, Node } from '@tensorflow/tfjs-converter/dist/operations/types';
...
export function getExecutionSubgraph(
  ...
  let dynamicNode: Node | null = null;
  let syncInputs: string[] | null = null;
  ...
  if (!dynamicNode) {
    throw new Error("Dynamic node is null");
  }

  if (!syncInputs) {
    throw new Error("Sync inputs is null");
  }
...
export function getNodesInTopologicalOrder(
  ...
  // inCounts tracks the number of incoming edges (dependencies) for each node
  // A node can only be executed once all its dependencies are satisfied (count reaches 0)
  const inCounts: Record<string, number> = {};
  for (const node of allNodes) {
    // Initialize count for this node if not already set
    inCounts[node.name] = inCounts[node.name] || 0;

    // For each child node, increment its incoming edge count
    for (const child of node.children) {
      // When the child is unused, set count to infinity so it will
      // never be executed since count can never reach 0
      if (!isUsed(child)) {
  ...
  while (frontier.length > 0) {
    ...
    if (!nodeName) {
      throw new Error("getNodesInTopologicalOrder - Node name is null");
    }
  ...
  const orderedNodes = orderedNodeNames.map((name) => {
...
function filterPredefinedReachableNodes(
  ...
  while (stack.length > 0) {
    ...
    if (!nodeName) {
      throw new Error("filterPredefinedReachableNodes - Node name is null");
    }
    const node = nameToNode.get(nodeName)!;
    for (const child of node.children) {
...
function validateNodesExecutionOrder(
  ...
  for (const node of orderedNodes) {
    for (const child of node.children.filter(willBeExecuted)) {
      ...
      const orderFromNodeName = nodeNameToOrder.get(node.name);
      const orderFromChildName = nodeNameToOrder.get(child.name);

      if (!orderFromNodeName || !orderFromChildName) {
        throw new Error("Order from node or child name is null");
      }
    ...
    if (!isPredefined(node)) {
      for (const input of node.inputs) {
        ...
        const orderFromInputName = nodeNameToOrder.get(input.name);
        const orderFromNodeName = nodeNameToOrder.get(node.name);

        if (!orderFromInputName || !orderFromNodeName) {
          throw new Error("Order from input or node name is null");
        }
...
export function parseNodeName(
  ...
  // Taken from:
  // https://github.com/tensorflow/tfjs/blob/master/tfjs-converter/src/operations/executors/utils.ts
