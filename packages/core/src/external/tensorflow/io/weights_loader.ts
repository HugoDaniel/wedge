...
Original:
https://github.com/tensorflow/tfjs/blob/master/tfjs-core/src/io/weights_loader.ts
...
import { decodeWeights, myFetchFunc } from './io_utils';
...
export function weightsLoaderFactory(
  ...
  return async (
    ...
    const groupIndicesToFetch =
      groupIndicesToFetchMap.reduce((accumulator: number[], shouldFetch: boolean, i: number) => {
        if (shouldFetch) {
          accumulator.push(i);
        }
        return accumulator;
