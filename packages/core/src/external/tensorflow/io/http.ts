...
Original:
https://github.com/tensorflow/tfjs/blob/master/tfjs-core/src/io/http.ts
...
export class HTTPRequest implements IOHandler {
  ...
  private readonly weightUrlConverter: ((weightName: string) => Promise<string>) | undefined;
...
export const httpRouter: IORouter =
  (url: string | string[], loadOptions?: LoadOptions) => {
    if (typeof fetch === 'undefined' &&
      ...
      if (isHTTP) {
        if (Array.isArray(url)) {
          throw new Error('HTTP router does not currently support array of URLs');
        }
